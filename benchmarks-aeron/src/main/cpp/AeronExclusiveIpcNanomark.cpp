...
class AeronIpcExclusiveNanomark : public nanomark::Nanomark
{
public:
    void setUp() override
    {
        ...
        for (std::size_t i = 0; i < MAX_THREAD_COUNT; i++)
        {
            bursters[i].reset(new Burster<ExclusivePublication>(
    ...
    std::unique_ptr<Burster<ExclusivePublication>> bursters[MAX_THREAD_COUNT];
};

SharedState AeronIpcExclusiveNanomark::sharedState;

NANOMARK(AeronIpcExclusiveNanomark, burst)(std::size_t id)
{
    Burster<ExclusivePublication> *burster = bursters[id].get();
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Burst Length = " << std::to_string(benchmarkBurstLength) << std::endl;
